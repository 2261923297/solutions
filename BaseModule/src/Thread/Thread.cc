...
thread_local tt::Thread* t_thread = nullptr;

tt::Thread*
tt::Thread::GetThis() { return t_thread; }

const std::string&
tt::Thread::GetName() { return t_thread_name; }

void
tt::Thread::SetName(const std::string& thread_name) { t_thread_name = thread_name; }
...
tt::Thread::Thread(std::function<void()> cb, const std::string& name) {
	m_cb = cb;
	m_name = name;
...
void* 
tt::Thread::run(void* thread_msg) {
	Thread* thread = (Thread*)thread_msg;

	t_thread = thread;
	t_thread_name = thread->m_name;
	thread->m_id = tt::GetThreadId();
