...
public:
	...
	void 
	getdirs(std::vector<std::string>& dirs, const std::string& path);

	void 
	setSplit(const char val) { m_dir_split = val; }

	const std::string& 
	absolute() const { return m_abs_path; }
	
	const char&
	split() const { return m_dir_split; }

	const std::string&
	current() const { return m_current_dir; }

	const std::string& 
	path() const { return m_ori_path; }
	...
	int 
	isAbs() const { return m_ori_path[0] == '/'; }
...
public:
	...
	void 
	resetPath(const std::string& path) { 
	...
	void 
	resetData(const std::string& path = "") { m_data->reset(path); }

	int
	reopen(const std::string& mode = "a+");
	...
	int 
	write(const char* buffer, size_t data, uint64_t pos = 0);

	int 
	read(char* buffer, size_t size, uint64_t pos = 0);
...
public:
	...
	int
	mkdir(const std::string& dirName, mode_t mode = 0700) {
		return DirMaker::mkdir(dirName, mode);
	}
	...
	size_t 
	getDirSize(const std::string& dirName) { return 0; }
